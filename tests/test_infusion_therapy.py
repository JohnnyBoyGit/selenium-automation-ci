...
class TestInfusionTherapyPage:  # Independent Medical Exams Page
    ...
    def test_infusion_therapy_page_title(self, infusion_therapy):
        ...
        # Visibility check
        assert infusion_therapy.element_is_visible(infusion_therapy.INFUSION_TITLE), "Title not visible"
        
        # Content check
        actual_title = infusion_therapy.driver.find_element(*infusion_therapy.INFUSION_TITLE).text
    ...
    def test_phone_data_integrity(self, infusion_therapy):
        ...
        infusion_therapy.navigate()
        assert infusion_therapy.element_is_visible(infusion_therapy.PHONE), "Phone element not visible"
        
        raw_phone = infusion_therapy.driver.find_element(*infusion_therapy.PHONE).text
    ...
    def test_address_data_integrity(self, infusion_therapy):
        ...
        infusion_therapy.navigate()
        assert infusion_therapy.element_is_visible(infusion_therapy.ADDRESS), "Address element not visible"
        
        raw_address = infusion_therapy.driver.find_element(*infusion_therapy.ADDRESS).text
    ...
    def test_icons_visibility(self, infusion_therapy):
        ...
        infusion_therapy.navigate()
        assert infusion_therapy.element_is_visible(infusion_therapy.PHONE_ICON), "Phone icon is not visible"
        assert infusion_therapy.element_is_visible(infusion_therapy.ADDRESS_ICON), "Address icon is not visible"
    ...
    def test_schedule_now_link(self, infusion_therapy):        
        ...
        CommonLogic.verify_link_flow(page=infusion_therapy, locator=infusion_therapy.SCH_NOW_BUTTON, expected_slug="booking", click_type="safe", scroll=True)
