...
class TestElectroDiagnosticsPage:
    def test_electrodiagnostics_page_title(self, electro_diagnostics):
        ...
        actual_title = electro_diagnostics.driver.find_element(*electro_diagnostics.ELECTRODIAG_TITLE).text
        
        # Verify visibility and content
        assert electro_diagnostics.element_is_visible(electro_diagnostics.ELECTRODIAG_TITLE)
    ...
    def test_phone_data_integrity(self, electro_diagnostics):
        ...
        assert electro_diagnostics.element_is_visible(electro_diagnostics.PHONE), "Phone is not visible"
        
        # 2. Now that we know it's visible, get the text
        raw_phone = electro_diagnostics.driver.find_element(*electro_diagnostics.PHONE).text
    ...
    def test_address_data_integrity(self, electro_diagnostics):
        ...
        electro_diagnostics.navigate()
        assert electro_diagnostics.element_is_visible(electro_diagnostics.ADDRESS), "Address is not visible"
        
         # Get the raw text of the address and validate it using the Logic Layer
        raw_address = electro_diagnostics.driver.find_element(*electro_diagnostics.ADDRESS).text
    ...
    def test_icons_visibility(self, electro_diagnostics):
        ...
        electro_diagnostics.navigate()
        assert electro_diagnostics.element_is_visible(electro_diagnostics.PHONE_ICON), "Phone icon is not visible"
        assert electro_diagnostics.element_is_visible(electro_diagnostics.ADDRESS_ICON), "Address icon is not visible"
    ...
    def test_schedule_now_link(self, electro_diagnostics):        
        ...
        CommonLogic.verify_link_flow(page=electro_diagnostics, locator=ElectroDiagnosticsPage.SCH_NOW_BUTTON, expected_slug="booking", click_type="safe", scroll=True)
